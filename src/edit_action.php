...
if(empty($name) || empty($subgenre) || empty($platform) || empty($age) || empty($developer) || empty($score) ) 
{
	...
	else //Se realiza la modificación de un registro de la BD. 
	{
		...
		$mysqli->query("UPDATE Videojuegos SET nombre = '$name', subgenero = '$subgenre', plataforma = '$platform', año_lanzamiento = '$age', desarrollador = '$developer',puntuacion = '$score' WHERE id = $idvideojuego");
		$mysqli->close();
